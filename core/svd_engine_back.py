...
class SVGEngine:
    ...
    def _generate_design_system(self, topic: str = "Technology", base_colors: List[str] = []) -> Dict:
        ...
        # 将原图提取到的杂乱颜色去重
        color_refs = ", ".join(list(set(base_colors))) if base_colors else "无"
        prompt = f"""你是资深 UI 设计师。正在重绘一张关于「{topic}」的技术图表。
我从原图中吸管提取了以下颜色：[{color_refs}]。
但这些颜色可能因为噪点偏灰暗。请基于这些基色，还原/推演出一套现代、清爽、高对比度的 SVG 配色方案。
输出严格的 JSON：
{{
    "background": "#HEX",
    "primary_box_fill": "#HEX",
    "secondary_box_fill": "#HEX",
    "table_header_fill": "#HEX",
    "text_color": "#HEX",
    "heading_font": "FontName",
    "body_font": "FontName"
}}
要求：配色需现代、高级、对比度适中，贴近原图视觉感受。仅输出 JSON。"""
        try:
            response = self.client.chat.completions.create(
                model="qwen-max",
                messages=[{"role": "user", "content": prompt}],
                temperature=0.3 # 降低温度，更忠实还原原图
            )
            self.design_system = self._extract_json(response.choices[0].message.content)
            logger.info(f"✅ 生成还原型设计系统: {self.design_system}")
            return self.design_system
        except Exception as e:
            logger.error(f"设计系统生成失败，使用默认学术色板: {e}")
            self.design_system = self.ACADEMIC_DESIGN_SYSTEM
            return self.design_system
    # =========================
...
需要切分的文本：{text}"""
    ...
    def _render_text(self, x: float, y: float, text: str,
        ...
        # =========================
        is_formula = False
        # 启发式规则：包含上下标符号、常见希腊字母或数学运算符
        formula_signals = r'[\^_]|alpha|beta|gamma|delta|epsilon|theta|lambda|mu|sigma|omega|sum|int|prod|sqrt|rightarrow|infty'
        if re.search(formula_signals, text, re.IGNORECASE):
            is_formula = True
            # 🚨 核心修复：SVG 属性中必须使用单引号 ' 包裹带空格的字体名，避免双引号嵌套报错
        ...
        def parse_formula_line(line_str: str) -> str:
            ...
            for part in parts:
                ...
                if sup_sub_match and is_formula:
                    ...
                    tspan_parts.append(f'<tspan font-style="{font_style}">{escaped_part}</tspan>')
        ...
        # 🚨 核心修复：确保 font-family 的单引号不会被 _escape_text 转义
        return f'''
    <text x="{x}" y="{start_y}"
...
"""
    ...
    def create_svg(self, layout_data: Dict, output_path: str):
        ...
        for el in elements:
            ...
            if etype == "box":
                # 根据语义决定使用主色还是副色
                is_primary = "unified" in el.get("label", "").lower() or "optimization" in el.get("label", "").lower()
                fill = ds["primary_box_fill"] if is_primary else ds["secondary_box_fill"]
                box_parts = []
                if shape == "circle":
            ...
            # =========================
                ...
                svg_parts.append(
                    self._render_text(xmin + w/2, ymin + h/2, wrapped_text, safe_size, 
                                        ds["text_color"], alignment, font_family=font_fam)
                )
            # =========================
            # line
            # =========================
            ...
            # =========================
                ...
                chart_parts = []
                structure = el.get("structure", {})
                fill = ds["secondary_box_fill"] # 坐标系使用副色
                ...
                for idx, item in enumerate(scatter_items):
                    ...
                    chart_parts.append(f'<circle cx="{item_x}" cy="{item_y}" r="18" fill="#FFFFFF" stroke="#000" stroke-width="1.5"/>')
                    chart_parts.append(self._render_text(item_x, item_y, item, 11, ds["text_color"], "center", font_family=ds["body_font"]))
                svg_parts.append(f'<g>\n' + "\n".join(chart_parts) + '\n</g>')
